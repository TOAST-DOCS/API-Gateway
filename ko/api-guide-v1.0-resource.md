...
| swaggerData.paths.{path}.{operation}.consumes                                                    | Array   | 선택    | 없음              | 없음                                                                                                  | API Gateway 리소스 요청 파라미터 > 콘텐츠 타입 목록 영역.                                                                                             |
...
| swaggerData.paths.{path}.{operation}.produces                                                    | Array   | 선택    | 없음              | 없음                                                                                                  | API Gateway 리소스 응답 > 콘텐츠 타입 목록 영역.                                                                                                  |
...
| swaggerData.paths.{path}.{operation}.x-nhncloud-apigateway.plugins                               | Object  | 선택??? | 없음              | 없음                                                                                                  | API Gateway 사용자 정의 플러그인 객체 영역.                                                                                                      |
| swaggerData.paths.{path}.{operation}.x-nhncloud-apigateway.plugins.{pluginCode}                  | Object  | 선택??? | 없음              | {pluginCode} HTTP, MOCK, CORS, SET_REQUEST_HEADER, SET_RESPONSE_HEADER, ADD_REQUEST_QUERY_PARAMETER | [리소스 플러그인 타입 Enum 코드](./enum-code/#???) 참고. [리소스 플러그인 타입별 JSON 설정값](???) 참고.                                                         |
