...
1. 좌측 리소스 트리에서 리소스를 추가할 경로에 마우스를 올리면 + 버튼이 표시됩니다. 버튼을 클릭한 후 **리소스 생성**을 선택합니다.
...
- **백엔드 엔드포인트 타입: HTTP(S)**
    - 백엔드 엔드포인트 URL 경로: 수신된 API 요청을 전달할 백엔드 엔드포인트 서비스의 API URL을 설정합니다.
        - 루트(/)로 시작해야 합니다.
        - 경로에는 리소스에서 생성한 [컨텍스트 변수](./console-guide-gov/#context-variables)를 설정할 수 있습니다.
- **백엔드 엔드포인트 타입: 사용자 정의 응답**
    - 사용자 정의 응답을 설정합니다. 
    - HTTP 상태 코드: 응답 HTTP 상태 코드를 입력합니다. (필수)
    - 헤더: 응답 헤더의 이름과 값을 입력합니다.
    - 응답: 응답 본문을 입력합니다.
    - 헤더와 응답 본문에는 리소스에서 생성한 [컨텍스트 변수](./console-guide-gov/#context-variables)를 설정할 수 있습니다.
...
- 메서드 수정 
    ...
    2. 메서드 이름, 메서드 설명, 백엔드 엔드포인트 URL 경로 항목은 수정할 수 있습니다. 
    3. 수정을 하려면 좌측 리소스 트리에서 메서드를 선택합니다.
    4. 설정을 변경한 후 **저장** 버튼을 클릭합니다.
...
> **[주의] 리소스 경로 삭제** <br>
> 리소스 경로를 삭제하면 선택된 리소스의 경로의 하위 리소스 경로와 메서드가 모두 삭제됩니다.
...
### 리소스 플러그인 추가와 삭제 { #adddelete-plugin }
...
- **플러그인 추가**
    1. 추가할 플러그인을 활성화합니다.
    2. 추가할 플러그인의 설정 내용을 입력한 후 **저장** 버튼을 클릭합니다.
    ...
    4. **저장** 버튼을 클릭합니다.
- **플러그인 삭제**
	1. 선택한 경로와 함께 하위 경로 및 메서드에서도 플러그인을 일괄 삭제하려면 **하위 경로와 메서드에 덮어쓰기**를 체크합니다.
	2. 삭제할 플러그인을 비활성화합니다.
	3. 변경 사항을 적용하려면 **저장** 버튼을 클릭합니다. 

> **[주의] 플러그인 추가 후 변경사항 저장** <br>
> 플러그인을 추가한 후 **저장**을 클릭해야 설정이 저장됩니다.


<a id="request-parameters"></a>
### 요청 파라미터 { #request-parameters }
리소스 메서드별 요청 파라미터와 응답 형식, 콘텐츠 타입을 설정합니다.
설정한 내용은 [API 설명서](./console-guide-gov/#api-documentation)에 적용됩니다.

1. 리소스 메서드를 선택합니다.
2. **요청 파라미터** 메뉴를 클릭합니다.
3. 요청 파라미터 항목을 입력합니다. 
    - 항목을 추가하려면, 테이블 상단에 위치한 **+추가 버튼**을 클릭합니다.
    ...
    - Raw 데이터
        ...
        - 설명: 파라미터의 설명을 입력합니다.
        - 모델: 요청 본문의 [모델](./console-guide-gov/#model)을 선택합니다.
    - 콘텐츠 타입
...
1. 리소스 메서드를 선택합니다.
2. **응답** 섹션을 클릭해 펼칩니다.
3. 응답 HTTP 상태 코드에서 **추가** 버튼을 클릭하면 입력 화면이 표시됩니다.
4. 추가할 응답 HTTP 상태 코드와 설명을 입력합니다.
6. 추가된 응답 HTTP 상태 코드 행의 왼쪽에 있는 화살표 아이콘을 클릭하여, 해당 상태 코드에 대한 응답 헤더와 응답 본문을 입력합니다.
    - 헤더
        - 이름: 헤더의 이름을 입력합니다. 
        - 설명: 헤더의 설명을 입력합니다.
...
- **요청 헤더 변경 설정** 
    ...
    - 헤더 이름과 값을 입력합니다. 
    - 헤더값에는 리소스에서 선언한 [컨텍스트 변수](./console-guide-gov/#context-variables)를 설정할 수 있습니다.
...
- **\+** 버튼을 클릭하면 헤더 목록을 추가할 수  있습니다.
- 헤더 이름과 값을 입력합니다. 
- 헤더값에는 리소스에서 선언된 [컨텍스트 변수](./console-guide-gov/#context-variables)를 설정할 수 있습니다.
...
- 파라미터 이름과 값을 입력합니다. 
- 파라미터 값에는 리소스에서 선언된 [컨텍스트 변수](./console-guide-gov/#context-variables)를 설정할 수 있습니다.
...
3. 삭제 확인 창에서 **확인**을 클릭하면 삭제됩니다. 이 작업은 취소할 수 없습니다. 
...
5. **배포** 버튼을 클릭합니다. 
6. 스테이지 이름 옆 아이콘을 통해 배포 상태를 확인할 수 있습니다.
    -  **초록색** 상태이면 정상적으로 배포된 상태입니다.
    -  **빨강색** 상태이면 배포 중 작업 오류가 발생한 상태입니다. 배포 실패가 발생하면 배포를 재시도해 주시기 바랍니다. 지속적으로 실패하는 경우 고객센터로 문의해주시기 바랍니다.
...
자세한 내용은 [API 설명서](./console-guide-gov/#api-documentation)를 참고합니다.
...
#### HMAC 인증을 위한 API 클라이언트 작업 { #authentication-hmac-api-client-actions-for-hmac-authentication }
...
#### SignToString 형식 { #authentication-hmac-signtostring-format }
```
[HTTP Method]\n
[Request URL]\n
[Request datetime]\n
...
#### SignToString 예시 { #authentication-hmac-signtostring-example }
...
2. **설정** 탭을 선택합니다.
3. 스테이지 트리 화면에서 스테이지의 루트(/) 경로를 선택합니다. 
4. 액세스 로그에서 **설정**을 선택하고 활성화(on)합니다. 
5. Log & Crash Search 서비스를 이용 중이지 않은 경우, Log & Crash Search 서비스를 활성화합니다. 
6. **수정** 버튼을 클릭하여 설정을 저장합니다.
7. 다시 **스테이지 배포**를 한 이후부터 액세스 로그가 Log & Crash Search 서비스에 보관됩니다. 
...
> <a class='text-guide' target='_blank' href='https://www.gov-nhncloud.com/kr/service/data-analytics/log-crash-search'>Log & Crash Search 서비스 소개 바로 가기</a>
...
자세한 내용은 [요청 제한 정책](./console-guide-gov/#request-restriction-policy)을 참고하세요.

1. **스테이지** 탭에서 스테이지를 선택합니다.
2. **설정** 탭을 선택합니다.
3. 스테이지 트리 화면에서 요청 제한 정책을 적용할 경로나 메서드를 선택합니다.
4. 요청 제한 정책에서 **설정**을 선택합니다.
...
- 사용량 계획의 스테이지에 연결된 API Key만 스테이지의 API를 요청할 수 있습니다. 자세한 방법은 [사용량 계획 > 사용량 계획에 스테이지 연결](./console-guide-gov/#connect-stage-to-usage-plan)을 참고하세요.)
- API Key가 연결된 사용량 계획의 요청 한도를 검사합니다. 사용량 계획의 요청 한도를 설정하는 자세한 방법은 [사용량 계획 > 사용량 계획 생성](./console-guide-gov/#create-usage-plan)을 참고하세요.)
...
4. API Key에서 **설정**을 선택하고 활성화(on)합니다.
5. 스테이지를 배포합니다.
...
6. 요청 유효성 검사기에서 **설정**을 선택하고 활성화(on)합니다.
7. 스테이지를 배포합니다.
...
1. 요청 제한 정책 목록에서 수정할 요청 제한 정책의 **관리** 열에서 연필 아이콘을 클릭합니다.
...
2. 하단 탭에서 **요청 제한 정책 키 값 생성** 버튼을 클릭합니다. 
...
1. API Gateway 서비스 목록에서 서비스 설정 열의 **리소스**를 클릭합니다.
2. **게이트웨이 응답** 탭에서 설정하고자 하는 게이트웨이 응답 유형의 관리탭에서 **연필 아이콘**을 클릭합니다.
3. 응답 정보를 입력합니다. 
    ...
    - 헤더 값과 응답 본문에는 [컨텍스트 변수](./console-guide-gov/#context-variables)를 설정할 수 있습니다.
...
- 게이트웨이 응답 유형은 [게이트웨이 응답 유형 Enum 코드](./enum-code-gov/#gateway-response-type) 문서를 참고합니다. 
...
> - 일반 응답 타임아웃은 최대 60초입니다. 백엔드 엔드포인트 서비스에서 응답이 지연되는 경우 타임아웃이 발생할 수 있습니다.
> - HTTP Streaming(SSE/chunked) 응답은 전체 응답 시간을 최대 10분까지 유지할 수 있습니다. 단, 백엔드의 청크 데이터 간 전송 간격이 60초를 초과하면 연결이 종료됩니다. 즉, 전체 응답 시간(10분) 또는 청크 데이터 간 전송 간격(60초) 중 하나라도 초과하면 연결이 강제 종료됩니다.
...
> - HTTP Streaming(SSE/chunked) 응답은 전체 응답 시간이 60초를 초과하더라도 정상 응답으로 처리되어 최대 10분까지 유지되며, 백엔드 엔드포인트가 데이터를 전송하지 않는 간격이 60초를 초과하는 경우 응답 지연으로 판단하여 연결을 종료합니다.

<a id="api-gateway-service-security-settings-and-network-communication"></a>
## API Gateway 서비스의 보안 설정과 네트워크 통신 허용 { #api-gateway-service-security-settings-and-network-communication }

<a id="api-gateway-service-security-settings"></a>
### API Gateway 서비스의 보안 설정 { #api-gateway-service-security-settings }
...
<a id="network-communication-between-api-gateway-service-and-endpoint"></a>
### API Gateway 서비스와 엔드포인트 간 네트워크 통신 허용 { #network-communication-between-api-gateway-service-and-endpoint }
...
3. **스테이지 적용** 버튼을 클릭하여 스테이지에 변경된 리소스 내용을 적용합니다.
4. 스테이지 설정 페이지의 **배포** 버튼을 클릭합니다. 
...
- **통계 그래프**
    - API 호출
      ▫ 성공: 응답된 HTTP 상태 코드가 2XX, 3XX인 API 호출 수
      ▫ 실패: 응답된 HTTP 상태 코드가 4XX, 5XX인 API 호출 수
...
- **API 호출**
    - **성공** : 응답된 HTTP 상태 코드가 2XX, 3XX인 API 호출 수 
    - **실패** : 응답된 HTTP 상태 코드가 4XX, 5XX 인 API 호출 수 
...
1. 사용량 계획 목록에서 해제할 사용량 계획이 름 열의 **이름** 링크를 클릭합니다.
...
> **[참고] 선택한 스테이지가 연결된 다른 사용량 계획으로만 변경할 수 있습니다.** <br>


> **[주의] 사용량 계획 변경 시 API Key 요청 할당량의 사용량 초기화** <br>
...
> **[주의] 사용자 정의 Primary/Secondary API Key** <br>
...
4. 확인 창에서 **해제** 버튼을 클릭합니다. 삭제된 데이터는 복구할 수 없습니다.


> **[주의] 사용자 지정 도메인 삭제 불가** <br> 
...
4. 확인 창에서 **확인** 버튼을 클릭합니다. 연결이 해제된 도메인은 미연결 도메인으로 이동합니다. 


> **[주의] 스테이지 연결 해제 시 사용자 지정 도메인으로 API 호출 불가** <br> 
...
> **[참고] /etc/hosts 파일 수정** <br>
> /etc/hosts 파일을 수정하지 않으면 스테이지 URL로 API 호출이 불가합니다.
> 하나의 서비스 게이트웨이 IP로 여러 개의 API Gateway 스테이지 도메인을 등록하여 사용할 수 있습니다.