...
자세한 내용은 [요청 제한 정책](/.console-guide-gov/#_31)을 참고하세요.
...
3. 응답 정보를 입력합니다. 
    ...
    - 헤더 값과 응답 본문에는 컨텍스트 변수를 설정할 수 있습니다. (자세한 컨텍스트 변수는 [컨텍스트 변수](./console-guide/#_11)를 참고하세요.)
    
> **[참고] 게이트웨이 응답 적용**
> 변경된 게이트웨이 응답은 스테이지 배포 이후에 배포 시점의 형상으로 적용됩니다. 

### 게이트웨이 응답 초기화
1. 게이트웨이 응답 목룍에서 초기화하려는 오류 유형을 선택합니다.
2. **기본값 초기화** 버튼을 클릭합니다.
3. 확인 창에서 **확인** 버튼을 클릭합니다. 삭제된 데이터는 복구할 수 없습니다.

### 게이트웨이 응답 유형
- 게이트웨이 응답 유형은 [게이트웨이 응답 유형 Enum 코드](./enum-code/#_7) 문서를 참고합니다. 
...
쉽게 리소스를 이전하려면 [스테이지 > 리소스 가져오기](./console-guide-gov/#_20)를 통해 스테이지에 등록된 리소스를 Swagger파일로 다운로드한 후, [리소스 > 리소스 가져오기](./console-guide-gov/#_3)를 통해 다운로드한 Swagger 파일로 리소스를 등록할 수 있습니다. 
...
1. 판교와 평촌 리전의 각 API Gateway 스테이지에 생성한 사용자 지정 도메인을 연결합니다. 자세한 방법은 [사용자 지정 도메인을 스테이지에 연결 가이드](./console-guide-gov/#_53)를 참고하세요.
2. 사용자 지정 도메인으로 API가 정상적으로 호출되는지와 GSLB 구성에 따라 트래픽이 분산되는지 확인합니다. 


## API Gateway 서비스 게이트웨이 연동 (판교 리전)
서비스 게이트웨이를 이용하면 NHN Cloud 내부 네트워크 내 클라이언트와 API Gateway 간 통신을 인터넷을 경유하지 않고 내부 네트워크를 통해 할 수 있습니다. 
