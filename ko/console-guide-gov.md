...
2. Swagger **JSON 파일 선택** 버튼을 클릭하여 파일을 선택하거나 Swagger 내용을 직접 입력합니다.
3. **적용** 버튼을 클릭합니다.
...
- 아이콘을 클릭하고 확인 창에서 확인을 선택하면 리소스가 삭제되며, 삭제된 데이터는 복구할 수 없습니다.
...
- **플러그인 추가**
    - 백엔드 요청 사전 작업과 프런트엔드 응답 사전 작업의 **플러그인 추가** 버튼을 클릭합니다.
        1. 추가할 플러그인을 클릭합니다.
        ...
        4. **저장** 버튼을 클릭합니다. 
- **플러그인 삭제**
	1. 백엔드 요청 사전 작업 또는 프런트엔드 응답 사전 작업의 플러그인 영역에서 우측 상단의 **휴지통** 아이콘을 클릭합니다.
	2. 선택한 경로와 함께 하위 경로 및 메서드에서도 플러그인을 일괄 삭제하려면 하위 경로와 메서드에 덮어쓰기를 체크합니다.
...
### API 문서
**스테이지 배포**를 통해 배포된 형상을 API 문서를 통해 확인 할 수 있습니다.
자세한 내용은 [API 문서](./console-guide-gov/#api_2)를 참고합니다.
...
5. Log & Crash Search 서비스를 이용 중이지 않은 경우, **서비스 활성화**를 클릭하여 서비스를 활성화합니다. 
...
1. 요청 제한 목록에서 수정할 요청 제한 정책의 관리 열에서 **연필** 아이콘을 클릭합니다.
...
5. 스테이지 설정 페이지의 **API 문서 탭**을 클릭합니다.
6. API 설명서의 게시 유형을 선택합니다.
    - **게시 유형**: API 문서의 접근 제한을 설정할 수 있습니다.
        - **공개**: 접근 제한 없이 API 문서에 접근이 가능합니다.
        - **비공개**: 콘솔에 접근 가능한 사용자만 API 문서에 접근할 수 있습니다.
7. **게시 링크**에 나타나는 URL로 API 문서를 확인할 수 있습니다. 

> **[참고] 스테이지 배포와 API 문서 게시** <br>
> - API 문서는 최초 스테이지 배포 이후 게시되며, 최초 게시 유형은 비공개로 설정됩니다.
> - API 문서는 스테이지 배포 수행 시 배포되는 형상으로 업데이트됩니다.
>
> **[주의] 게시된 API 문서에서 API 호출 테스트 시 CORS 설정** <br>
> - 게시된 API 문서 도메인 주소와 호출하는 API의 도메인 주소가 다르기 때문에 API 문서 내에서 호출을 테스트하고자 하는 경우 CORS 설정이 필요할 수 있습니다.
> - 예:
...
5. 오른쪽의 **스테이지 통계** 탭에서는 스테이지의 통계 지표를 확인할 수 있습니다. 
6. 오른쪽의 **리소스 통계** 탭에서는 HTTP 메서드와 경로별 통계 지표를 확인할 수 있습니다. 
...
### 사용량 계획 수정
1. 사용량 계획 목록에서 수정할 사용량 계획의 관리 열에 있는 **연필 아이콘**을 클릭합니다.
3. 사용량 계획 정보를 수정합니다.
3. 변경한 내용을 저장하려면 **수정** 버튼을 클릭합니다.
...
### 사용량 계획 삭제
1. 사용량 계획 목록에서 삭제할 사용량 계획의 관리 열에 있는 *휴지통 아이콘**을 클릭합니다.
3. 확인 창에서 **확인** 버튼을 클릭합니다. 삭제된 데이터는 복구할 수 없습니다.
...
### API Key 수정
1. API Key 목록에서 수정할 API Key의 관리 열에 있는 **연필 아이콘**을 클릭합니다.
...
### API Key 삭제
1. API Key 목록에서 삭제할 API Key의 관리 열에 있는 **휴지통 아이콘**을 클릭합니다.
3. 확인 창에서 **확인** 버튼을 클릭합니다. 삭제된 데이터는 복구할 수 없습니다.
...
4. GSLB 도메인: API Gateway 리전 이중화 구성을 하려면 GSLB 도메인을 입력합니다. API Gateway 리전 이중화 가이드에 대한 자세한 내용은 [API Gateway 리전 이중화 가이드](./console-guide-gov/#api-gateway_4)를 참고합니다. 
...
1. **사용자 지정 도메인** 메뉴로 이동합니다.
2. 삭제할 사용자 지정 도메인의 관리 열에 있는 **휴지통 아이콘**을 클릭합니다.
