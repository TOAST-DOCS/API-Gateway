...
## API Gateway 서비스의 보안을 위한 설정과 네트워크 통신 허용

### API Gateway 서비스의 보안을 위한 설정   
API Gateway에서 배포된 스테이지 URL은 인터넷에서 접근이 허용됩니다.
노출하지 않을 API는 API Gateway의 리소스로 등록하지 않도록 합니다.
리소스로 등록된 API에서 제한된 클라이언트만 API 요청을 허용하려면 다음의 설정을 참고하여 추가하시기 바랍니다.
* 접근 제어/인증 관련 설정
    * IP ACL: 제한된 클라이언트 IP에 대해 API 요청을 허부 하거나 거부합니다.
