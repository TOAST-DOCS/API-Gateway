...
## 사용자 지정 게이트웨이 응답
게이트웨이에서 정의된 오류 응답 설정을 변경할 수 있습니다. 

### 사용자 지정 게이트웨이 응답 생성, 수정
1. API Gateway 서비스 목록에서 서비스 설정 열의 **리소스**를 클릭합니다.
2. **사용자 지정 응답** 탭에서 설정하고자 하는 게이트웨이 응답 유형을 클릭합니다.
3. 응답 정보를 입력합니다. 
    ...
    - **응답 본문 템플릿**: 콘텐츠 타입과 그에 대응되는 응답 본문을 입력합니다.
        ...
        - 응답 본문: 요청의 Accept 헤더 값에 대응하여 반환할 사용자 지정 응답 본문을 입력합니다.
    - 헤더 값과 응답 본문에는 컨텍스트 변수를 설정할 수 있습니다. (자세한 컨텍스트 변수는 [컨텍스트 변수](./console-guide/#_11)를 참고하세요.)
    
> **[참고] 사용자 지정 게이트웨이 응답 적용**
> 변경된 사용자 지정 게이트웨이 응답은 스테이지 배포 이후에 배포 시점의 형상으로 적용됩니다. 

### 사용자 지정 게이트웨이 응답 초기화
1. 사용자 지정 응답 목룍에서 초기화하려는 오류 유형을 선택합니다.
2. **초기화** 버튼을 클릭합니다.
3. 확인 창이 나타나면 **확인** 버튼을 클릭합니다. 삭제된 데이터는 복구할 수 없습니다.
