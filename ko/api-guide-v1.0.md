...
| resourceList[2].resourcePluginList[0].pluginConfigJson | Object   | [리소스 플러그인 타입별 JSON 설정값](#resource-plugin) 참고                   |
...
- 리소스 메서드는 선택 입력입니다. 생성된 리소스 경로의 하위에 메서드를 추가하려면 [리소스 메서드 생성](#create-resource-methods) API를 사용해야합니다.
...
| resourcePathList[0].pathPluginList[0].pluginConfigJson | Object | 필수 | 없음 | 없음 | [리소스 플러그인 타입별 JSON 설정값](#resource-plugin) 참고.|
...
| resourcePathList[0].methodList[0].methodPluginList[0].pluginConfigJson | Object | 필수 | 없음 | 없음 | [리소스 플러그인 타입별 JSON 설정값](#resource-plugin) 참고.|
...
| resourceList[1].resourcePluginList[0].pluginConfigJson | Object   | [리소스 플러그인 타입별 JSON 설정값](#resource-plugin) 참고                   |
...
| methodList[0].methodPluginList[0].pluginConfigJson | Object | 필수 | 없음 | 없음 | [리소스 플러그인 타입별 JSON 설정값](#resource-plugin) 참고.|
...
| resourceList[0].resourcePluginList[0].pluginConfigJson | Object   | [리소스 플러그인 타입별 JSON 설정값](#resource-plugin) 참고                   |
...
- 리소스 경로에 설정 가능한 플러그인만 설정할 수 있습니다. 자세한 내용은 [리소스 플러그인](#resource-plugin)을 참고합니다.
...
| pathPluginList[0].pluginConfigJson | Object | 조건부 필수 | 없음 | 없음 | [리소스 플러그인 타입별 JSON 설정값](#resource-plugin) 참고, delete 필드가 false인 경우 필수 입력|
...
- delete 필드를 true로 설정하면, 요청한 플러그인 타입의 플러그인이 삭제됩니다. delete 필드가 true이면 pluginConfigJson 필드는 정의하지 않아도 됩니다.
- 리소스 메서드에 설정 가능한 플러그인만 설정할 수 있습니다. 자세한 내용은 [리소스 플러그인](#resource-plugin)을 참고합니다.
...
| methodPluginList[0].pluginConfigJson | Object | 조건부 필수 | 없음 | 없음 | [리소스 플러그인 타입별 JSON 설정값](#resource-plugin) 참고, delete 필드가 false인 경우 필수 입력|
...
| swaggerData.paths.{path}.{operation}.x-nhncloud-apigateway.plugins.{pluginCode} | Object | 필수 | 없음 | {pluginCode} HTTP, MOCK, CORS, SET_REQUEST_HEADER, SET_RESPONSE_HEADER, ADD_REQUEST_QUERY_PARAMETER | [리소스 플러그인 타입 Enum 코드](./enum-code/#resource-plugin-type) 참고. [리소스 플러그인 타입별 JSON 설정값](#resource-plugin) 참고. |
...
* 스테이지 리소스 플러그인에 대한 자세한 내용은 [스테이지 리소스 플러그인](#stage-resource-plugin)을 참고합니다.
...
|stageResourceList[0].stageResourcePluginList[0].pluginConfigJson       |Object  |[리소스 플러그인 타입](#resource-plugin), [스테이지 플러그인 타입](#stage-resource-plugin)별 설정 JSON 참고            |
...
* 스테이지 리소스 플러그인에 대한 자세한 정보는 [스테이지 리소스 플러그인](#stage-resource-plugin)을 참고합니다.
...
| stageResourcePluginList[0].pluginConfigJson | Object | 필수 | 없음 | 없음 | 스테이지 리소스 플러그인 별 JSON 형식의 객체<br>[스테이지 플러그인 타입](#stage-resource-plugin) 참고|
...
- 스테이지 배포 요청 후, 스테이지 배포 성공 여부는 [최근 스테이지 배포 결과 조회](#query-result-of-recent-stage-deployment)에서 확인할 수 있습니다. 
...
- [스테이지 배포](#deploy-stage)의 결과를 조회할 수 있습니다. 
...
|latestStageDeployResult.stageResourceList[0].stageResourcePluginList[0].pluginConfigJson       |Object  |[리소스 플러그인 타입](#resource-plugin), [스테이지 플러그인 타입](#stage-resource-plugin)별 설정 JSON 참고          |
