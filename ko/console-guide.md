...
2. 스테이지 트리 화면에서 스테이지의 루트(/) 경로 또는 메서드를 선택합니다.
    - 루트 경로에서 설정한 요청 수 제한은 하위 정의된 모든 메서드 호출에 대해서 적용됩니다.
    - 메서드에서 설정한 요청 수 제한은 해당 메서드를 호출 시 적용되며, 루트 경로에서 설정된 요청 수 제한은 적용되지 않습니다.  
3. 요청 수 제한을 **활성화(On)** 합니다. 
4. 요청 수 제한을 설정합니다. 
    - 초당 요청 수: 초당 호출 가능한 최대 요청 수를 입력합니다.
    - 사용자 지정 키: 사용자가 지정한 키에 따라 입력된 초당 요청 수가 적용됩니다.
        - None: 사용자 지정 키를 사용하지 않습니다. 사용자 지정 키를 사용하지 않는 경우, 요청 수 제한이 정의된 위치(2번)에 따라 적용됩니다.
        - 경로 변수: 선택된 메서드 경로 내에 경로 변수가 선언되어 있는 경우에만 지정할 수 있으며, 해당 경로 변수에 해당하는 값별로 초당 요청 수를 제한합니다. 
        - IP: 요청한 클라이언트 IP별로 초당 요청 수를 제한합니다. 
        - 헤더: 요청 헤더 이름을 입력하여, 입력된 헤더 이름에 해당하는 헤더 값을 기반으로 초당 요청 수를 제한합니다. 
...
>       
> **[주의] 사용자 지정 키**
> 사용자 지정 키를 사용하는 경우, 요청에 사용자 지정 키가 포함되어야 합니다. 예를 들어 사용자 지정 키로 헤더를 선택하고, X-NHN-CLOUD 값을 입력했다면, 요청 헤더에는 X-NHN-CLOUD 가 포함 되어야 합니다.     
> 요청에서 사용자 지정 키를 찾을 수 없는 경우, 요청 수 제한이 적용되지 않습니다.
>
> **[주의] 초당 요청수 정확도**
