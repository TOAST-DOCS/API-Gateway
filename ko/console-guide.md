...
| 헤더 이름 | 헤더 값 |
| --- | --- |
| Authroization | hmac algorithm="\<encrypt_algorithm\>", headers="\<validation_headers\>", signature="\<base64_digest\>" |
| x-nhn-date |  ISO datetime 형식의 요청 시간  (yyyy-MM-dd'T'HH:mm:ssZZ)| 

- Authroization 또는  x-nhn-date 헤더가 요청에 포함되지 않은 경우 HMAC 인증에 실패합니다.
