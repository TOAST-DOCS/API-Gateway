...
백엔드 엔드포인트를 호출하기 전에 별도 API 호출을 통한 인증이 필요한 경우 혹은 먼저 호출되어야하는 API가 존재하는 경우 등에 활용이 가능합니다.

1. 스테이지 탭에서 스테이지를 선택합니다.
2. 스테이지 트리 화면에서 사전 호출 API를 적용할 경로나 메서드를 선택합니다.
  - 루트 경로에서 설정한 사전 호출 API는 하위 정의된 모든 메서드 호출에 대해서 적용됩니다.
...
3. 사전 호출 API를 활성화(on)합니다.
  ...
  - 캐시 유효 시간을 0이 아닌 값으로 입력할 경우 입력된 숫자(초)만큼 사전 호출 API(Pre-call API)의 응답 결과가 캐싱됩니다.
