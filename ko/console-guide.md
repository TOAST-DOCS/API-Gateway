...
3. 인증에서 **JWT**을 선택합니다.
4. JWT 설정을 입력합니다. 
    - **토큰 암호화 알고리듬**: 토큰을 서명할 때 이용한 암호화 알고리듬을 선택합니다. 암호화 알고리듬은 HS256, RS256 을 지원합니다.
        - HS256 토큰 알고리듬 
...
3. 사전 호출 API를 활성화(on)합니다.
  ...
  >
  > **[참고] 사전 호출 API의 요청 본문**
  > 사전 호출 API를 호출할 때 API Gateway에 요청된 헤더값만이 사전 호출 API 요청 정보에 포함되며, 요청 본문(request body)은 사전 호출 API 요청 시 포함되지 않습니다.
  > 따라서 사전 호출 API에서 요청 정보에 따라 다른 응답 코드를 응답하기 위해서는 요청의 헤더값을 사용하여 판단하도록 설정하여야 합니다.
  >
### 백엔드 엔드포인트 URL 재정의 
