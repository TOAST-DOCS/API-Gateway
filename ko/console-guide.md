...
3. 스테이지 배포 이력을 확인할 수 있습니다.
    ...
    - **기반 스테이지**: 현재 스테이지 상태의 기반이 되는 배포 이력을 의미합니다. 스테이지 배포와 스테이지 되돌리기를 수행할 경우 변경됩니다. 
    - **스테이지 되돌리기**: 해당 배포 이력으로 스테이지 상태가 되돌아갑니다.
...
1. 스테이지 탭에서 스테이지를 선택합니다.
2. 설정 탭을 선택합니다.
3. 스테이지 트리 화면에서 백엔드 엔드포인트 URL을 재정의할 메서드를 선택합니다.
