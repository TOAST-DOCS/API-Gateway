...
## Application Service > API Gateway > コンソール使用ガイド { #application-service-api-gateway-console-user-guide }
...
### API Gatewayサービス作成 { #create-api-gateway-service }
...
### API Gatewayサービス照会 { #view-api-gateway-service }
...
- **バックエンドエンドポイントタイプ：HTTP(S)**
    - バックエンドエンドポイントURLパス：受信したAPIリクエストを伝達するバックエンドエンドポイントサービスのAPI URLを設定します。
        - ルート(/)で始まる必要があります。
        - パスにはリソースで作成した[コンテキスト変数](./console-guide/#context-variables)を設定できます。
- **バックエンドエンドポイントタイプ：ユーザー定義レスポンス**
    - ユーザー定義レスポンスを設定します。 
    - レスポンスステータスコード：レスポンスHTTPステータスコードを入力します。(必須)
    - ヘッダ：レスポンスヘッダの名前と値を入力します。
    - レスポンス本文：レスポンス本文を入力します。
    - ヘッダとレスポンス本文にはリソースで作成した[コンテキスト変数](./console-guide/#context-variables)を設定できます。
...
### リソースとメソッド修正 { #modify-resources-and-methods }
- リソースパス修正
...
### ステージリソース適用 { #apply-stage-resource }
...
- **プラグイン追加**
    1. 追加するプラグインを有効にします。
    2. 追加するプラグインの設定内容を入力した後、**保存**ボタンをクリックします。
    3. 選択したパスを含め、サブパスとメソッドにプラグインを一括適用するには、**サブパスとメソッドに上書き**をチェックします。
        - **!注意**: サブパスとメソッドに追加しようとするプラグインが既に登録されている場合、設定が上書きされるためご注意ください。
    4. **保存**ボタンをクリックします。
- **プラグイン削除**
	2. 選択したパスと共に、サブパス及びメソッドからもプラグインを一括削除するには、「サブパスとメソッドに上書き」をチェックします。
	1. 選択したパスと共に、サブパス及びメソッドからもプラグインを一括削除するには、**サブパスとメソッドに上書き**をチェックします。
    2. 削除するプラグインを無効にします。
    3. **削除**ボタンを押します。
    4. **変更内容保存ボタン**を押します。
...
設定した内容は[API説明書](./console-guide/#api-documentation)に適用されます。
...
3. リクエストパラメータ項目を入力します。 
    ...
    - リクエスト本文パラメータ
        ...
        - 説明：パラメータの説明を入力します。
        - モデル：リクエスト本文の[モデル](./console-guide/#model)を選択します。
    - コンテンツタイプ
        - サーバーに転送する文書のコンテンツタイプ(例：application/json)を入力します。 
4. 変更内容の**保存**ボタンをクリックします。
...
5. 追加されたレスポンスHTTPステータスコードの行を選択します。
...
### リクエストヘッダ変更 { #change-request-header }
リクエストヘッダを追加または変更します。 
...
- **リクエストヘッダ変更設定** 
    ...
    - ヘッダ名と値を入力します。 
    - ヘッダ値にはリソースで宣言した[コンテキスト変数](./console-guide/#context-variables)を設定できます。
...
### リクエストヘッダの削除 { #delete-request-header }
...
### レスポンスヘッダ変更 { #change-response-header }
...
- **\+**ボタンを押すとヘッダリストを追加できます。
- ヘッダ名と値を入力します。 
- ヘッダ値にはリソースで宣言された[コンテキスト変数](./console-guide/#context-variables)を設定できます。
...
### レスポンスヘッダの削除 { #delete-response-header }
...
### リクエストクエリ文字列パラメータ追加 { #add-request-query-string-parameter }
...
- パラメータ名と値を入力します。
- パラメータ値にはリソースで宣言された[コンテキスト変数](./console-guide/#context-variables)を設定できます。
...
1. 削除するステージを選択します。
2. **ステージ削除**ボタンを押します。
3. 削除確認ウィンドウで**削除**を押すと削除されます。この作業はキャンセルできません。 
...
### リソースのインポート { #stage-import-resource }
...
### ステージ配布履歴 { #stage-deployment-history }
...
### API説明書 { #api-documentation }
**ステージ配布**を行って配布された形状をAPI説明書で確認できます。
詳細は[API説明書](./console-guide/#api-documentation)を参照してください。

<a id="stage-plugin"></a>
## ステージプラグイン { #stage-plugin }

<a id="backend-endpoint-url-override"></a>
### バックエンドエンドポイントURL再定義 { #backend-endpoint-url-override }

API Gatewayに受信したリクエストをバックエンドエンドポイントへ伝達する時、基本的にステージに定義されたバックエンドエンドポイントURLにリクエストを伝達します。
特定のパスまたはメソッドに対してバックエンドエンドポイントURLを再定義するには、バックエンドエンドポイントURL再定義を設定します。
...
2. **設定**タブを選択します。
3. ステージツリー画面でステージのルート(/)パスを選択します。
4. アクセスログで**設定**を選択し、有効化(on)します。
5. Log & Crash Searchサービスを利用中でない場合、Log & Crash Searchサービスを有効にします。
...
6. リクエスト検証機能で**設定**を選択し、有効化(on)します。
7. ステージを配布します。
...
- 使用量計画のステージに接続されたAPI KeyのみステージのAPIをリクエストできます。 (詳細な方法は[使用量計画 > 使用量計画にステージ接続](./console-guide/#connect-stage-to-usage-plan)を参照してください。)
- API Keyが接続された使用量計画のリクエスト限度をチェックします。 (使用量計画のリクエスト限度を設定する詳しい方法は[使用量計画 > 使用量計画作成](./console-guide/#create-usage-plan)を参照してください。)
...
### リクエスト制限ポリシー { #request-restriction-policy }

登録されたリクエスト制限ポリシーをステージリソースパスまたはメソッドに適用します。
詳細は[リクエスト制限ポリシー](./console-guide/#request-restriction-policy)を参照してください。
...
### ステージリソースにリクエスト制限ポリシー適用 { #apply-request-restriction-policy-to-stage-resources }
1. **ステージ**タブでステージを選択します。
2. **設定**タブを選択します。
3. ステージツリー画面でリクエスト制限ポリシーを適用するパスやメソッドを選択します。
4. リクエスト制限ポリシーで**設定**を選択します。
...
### リクエスト制限キー値の作成 { #create-request-restriction-key-value }
...
### リクエスト制限キー値の修正 { #modify-request-restriction-key-value }
...
### リクエスト制限キー値の削除 { #delete-request-restriction-key-value }
...
### ゲートウェイレスポンスの再定義 { #redefine-gateway-response }
...
3. レスポンス情報を入力します。 
    ...
    - ヘッダ値とレスポンス本文には[コンテキスト変数](./console-guide/#context-variables)を設定できます。
...
- ゲートウェイレスポンスタイプは[ゲートウェイレスポンスタイプEnumコード](./enum-code/#gateway-response-type)文書を参考します。 
- ゲートウェイレスポンスタイプに基づく基本本文形式は[Gatewayエラーコード](./error-code/)文書を参照してください。 


<a id="check-api-call"></a>
## API呼び出し確認 { #check-api-call }
...
5. **ステージ統計**タブでは、ステージの統計指標を確認できます。 
6. **リソース統計**タブでは、HTTPメソッドとパスごとに統計指標を確認できます。 

<a id="note-on-statistical-data"></a>
### 統計データ参考事項 { #note-on-statistical-data }
...
### 使用量計画に接続されたステージの解除 { #disable-stage-connected-to-the-usage-plan }
1. 使用量計画リストから使用量計画名列の**名前**のリンクをクリックします。
...
### API Key接続解除 { #disconnect-api-key }
1. 使用量計画リストから使用量計画名列の**名前**のリンクをクリックします。
...
### API Keyの使用量計画の変更 { #change-the-usage-plan-of-api-key }
1. 使用量計画リストから使用量計画名列の**名前**のリンクをクリックします。
...
### API Keyの再発行 { #regenerate-api-key }
...
### API Keyと接続されたステージ { #stage-connected-to-api-key }
1. API KeyリストからAPI Keyを選択します。
...
### ユーザー指定ドメインの作成 { #create-custom-domain }
...
4. GSLBドメイン: API Gatewayリージョン冗長構成をするにはGSLBドメインを入力します。 API Gatewayリージョン冗長化ガイドの詳細については[API Gatewayリージョン冗長化ガイド](./console-guide/#api-gateway-region-redundancy)を参照してください。
...
### ユーザー指定ドメインの削除 { #delete-custom-domain }
...
### ユーザー指定ドメインのステージ接続 { #connect-stage-of-custom-domain }
...
### ユーザー指定ドメインのステージ接続解除 { #disconnect-stage-from-a-custom-domain }
...
## API Gatewayリージョン冗長化 { #api-gateway-region-redundancy }
...
### 1. 韓国((ピョンチョン)リージョンのサービスとステージ作成 { #service-and-create-stage-in-korea-pyeongchon-region }

1. 韓国((ピョンチョン)リージョンにAPI Gatewayサービスを作成します。
2. 韓国((ピョンチョン)リージョンのAPI Gatewayサービスに運営中の韓国((パンギョ)リージョンステージと同じリソースを登録します。  
簡単にリソースを以前するには[ステージ > リソースのインポート](./console-guide/#stage-import-resource)を通じてステージに登録されたリソースをSwaggerファイルでダウンロードした後、 [リソース > リソースのインポート](./console-guide/#import-resource)を通じてダウンロードしたSwaggerファイルでリソースを登録できます。 
...
### 3. ユーザー指定ドメインの作成 { #api-gateway-region-redundancy-create-custom-domain }
...
1. 韓国(パンギョ)と韓国(ピョンチョン)リージョンの各API Gatewayステージに作成したユーザー指定ドメインを接続します。詳細な方法は[ユーザー指定ドメインをステージに接続ガイド](./console-guide/#connect-custom-domain-to-stages-in-each-region)を参照してください。
2. ユーザー指定ドメインでAPIが正常に呼び出されるか、GSLB構成によってトラフィックが分散されるかを確認します。 


<a id="api-gateway-service-gateway-integration"></a>
## API Gatewayサービスゲートウェイ連動 { #api-gateway-service-gateway-integration }
