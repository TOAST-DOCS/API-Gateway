...
- パラメータ名と値を入力します。
- パラメータ値にはリソースで宣言されたコンテキスト変数を設定できます。 (コンテキスト変数の詳細は[コンテキスト変数](./console-guide/#_8)を参照してください。)
0
> **[参考]リクエストクエリ文字列パラメータ**
...
- 使用量計画のステージに接続されたAPI KeyのみステージのAPIをリクエストできます。 (詳細な方法は[使用量計画 > 使用量計画にステージ接続](./console-guide/#_31)を参照してください。)
- API Keyが接続された使用量計画のリクエスト限度をチェックします。 (使用量計画のリクエスト限度を設定する詳しい方法は[使用量計画 > 使用量計画作成](./console-guide/#_28)を参照してください。)
...
> **[注意]**使用量計画を変更しても既存API Keyの使用量は維持されます。使用量制限限度が低い使用量計画に変更すると、使用量が超過することがあります。
## API Key
