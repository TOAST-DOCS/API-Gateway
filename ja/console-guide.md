...
- API Keyが接続された使用量計画のリクエスト限度をチェックします。 (使用量計画のリクエスト限度を設定する詳しい方法は[使用量計画 > 使用量計画作成](./console-guide/#usage-plan)を参照してください。)
...
簡単にリソースを以前するには[ステージ > リソースのインポート](./console-guide/#import-resource)を通じてステージに登録されたリソースをSwaggerファイルでダウンロードした後、 [リソース > リソースのインポート](./console-guide/#import-resource)を通じてダウンロードしたSwaggerファイルでリソースを登録できます。 
