...
設定した内容は[API説明書](#api-documentation)に適用されます。
...
- **リクエストヘッダ変更設定** 
    ...
    - ヘッダ名と値を入力します。 
    - ヘッダ値にはリソースで宣言した[コンテキスト変数](#context-variables)を設定できます。
...
- **\+**ボタンを押すとヘッダリストを追加できます。
- ヘッダ名と値を入力します。 
- ヘッダ値にはリソースで宣言された[コンテキスト変数](#context-variables)を設定できます。
...
- パラメータ名と値を入力します。
- パラメータ値にはリソースで宣言された[コンテキスト変数](#context-variables)を設定できます。
...
詳細は[API説明書](#api-documentation)を参照してください。
...
- 使用量計画のステージに接続されたAPI KeyのみステージのAPIをリクエストできます。 (詳細な方法は[使用量計画 > 使用量計画にステージ接続](#connect-stage-to-usage-plan)を参照してください。)
- API Keyが接続された使用量計画のリクエスト限度をチェックします。 (使用量計画のリクエスト限度を設定する詳しい方法は[使用量計画 > 使用量計画作成](#create-usage-plan)を参照してください。)
...
詳細は[リクエスト制限ポリシー](#request-restriction-policy)を参照してください。
...
3. レスポンス情報を入力します。 
    ...
    - ヘッダ値とレスポンス本文には[コンテキスト変数](#context-variables)を設定できます。
...
4. GSLBドメイン: API Gatewayリージョン冗長構成をするにはGSLBドメインを入力します。 API Gatewayリージョン冗長化ガイドの詳細については[API Gatewayリージョン冗長化ガイド](#api-gateway-region-redundancy)を参照してください。
...
簡単にリソースを以前するには[ステージ > リソースのインポート](#stage-import-resource)を通じてステージに登録されたリソースをSwaggerファイルでダウンロードした後、 [リソース > リソースのインポート](#import-resource)を通じてダウンロードしたSwaggerファイルでリソースを登録できます。 
...
1. 韓国(パンギョ)と韓国(ピョンチョン)リージョンの各API Gatewayステージに作成したユーザー指定ドメインを接続します。詳細な方法は[ユーザー指定ドメインをステージに接続ガイド](#connect-stage-of-custom-domain)を参照してください。
