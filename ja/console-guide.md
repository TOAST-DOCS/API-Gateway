...
### リソース作成 
1. 左側のリソースツリーで、リソースを追加したいパスにマウスカーソルを合わせると**+**ボタンが表示されます。ボタンをクリックした後、**リソース作成**を選択します。
...
2. **JSONファイル選択**ボタンをクリックしてファイルを選択するか、Swaggerの内容を直接入力します。
3. **適用**ボタンをクリックします。
...
- **バックエンドエンドポイントタイプ：ユーザー定義レスポンス**
    - ユーザー定義レスポンスを設定します。 
    - HTTPステータスコード：レスポンスHTTPステータスコードを入力します。(必須)
...
    2. メソッド名、メソッドの説明、バックエンドエンドポイントのURLパス項目は修正できます。
    3. 修正をするには左側リソースツリーからメソッドを選択します。
    4. 設定を変更した後、**変更内容保存**ボタンを押します。

> **[参考] CORSプラグインにより登録されたOPTIONSメソッドの修正** <br>
> CORSプラグインにより登録されたOPTIONSメソッドは修正できません。 

### リソースとメソッド削除 
削除したいリソースパスまたはメソッドにマウスカーソルを合わせると、ごみ箱アイコンが表示されます。
ごみ箱アイコンをクリックし、確認ウィンドウで**確認**を選択するとリソースが削除され、削除されたデータは復元できません。
...
### ステージの適用
リソースを変更した後、ステージに変更内容を適用するには**ステージの適用**を行う必要があります。

1. 変更されたリソースをステージに適用するには**ステージ適用**ボタンを押します。
...
> - すでにステージに最新リソースが適用されている場合、ステージの適用ができません。
...
- **プラグイン追加**
    - バックエンドリクエスト事前作業とフロントエンドレスポンス事前作業の**+プラグイン追加**ボタンを押します。
        1. 追加するプラグインをクリックします。
...
        4. **保存**ボタンをクリックします。
- **プラグイン削除**
	1. バックエンドリクエストの事前タスク、またはフロントエンドレスポンスの事前タスクのプラグインエリアで、右上のごみ箱アイコンをクリックします。
	2. 選択したパスと共に、下位のパス及びメソッドからもプラグインを一括で削除するには、「下位パスとメソッドに上書き」にチェックを入れます。
	3. 変更内容を適用するには、**保存**ボタンをクリックします。

> **[注意]プラグイン追加後、変更事項を保存** <br>
> プラグインを追加した後、**保存**ボタンを押すと設定が保存されます。


### リクエストパラメータ 
リソースメソッド別リクエストパラメータとレスポンス形式、コンテンツタイプの設定を行います。
設定した内容は[APIドキュメント](./console-guide/#api_2)に適用されます。

1. リソースメソッドを選択します。
2. **リクエストパラメータ**メニューをクリックします。
3. リクエストパラメータ項目を入力します。 
    - 項目を追加するには、テーブル上部にある**+追加**をクリックします。
    - 項目を削除するには、項目の右側にある**-**ボタンをクリックします。
    - クエリ文字列、ヘッダ、フォームデータ
        - 名前：パラメータの名前を入力します。 
        - 説明：パラメータの説明を入力します。
...
1. リソースメソッドを選択します。
2. **応答**セクションをクリックして展開します。
3. 応答HTTPステータスコードで**追加**ボタンをクリックすると、入力画面が表示されます。
4. 追加するレスポンスHTTPステータスコードと説明を入力します。
5. **作成**ボタンをクリックします。
6. 追加された応答HTTPステータスコードの行の左側にある矢印アイコンをクリックし、該当のステータスコードに対するレスポンスヘッダとレスポンスボディを入力します。
    - ヘッダ
        - 名前：ヘッダの名前を入力します。 
        - 説明：ヘッダの説明を入力します。
...
- **プラグイン適用段階**：フロントエンドレスポンス事前作業
- **レスポンスヘッダ削除設定** 
    - **\+ 追加**ボタンをクリックするとヘッダリストを追加できます。
    - 削除するヘッダ名を追加します。

> **[参考]レスポンスヘッダの変更と削除設定** <br>
...
1. API Gatewayサービスリストで**ステージ**をクリックします。
2. ステージタブで **+ 作成**ボタンをクリックします。
...
5. **デプロイ**ボタンをクリックします。
6. ステージ名の横にあるアイコンを通じて、デプロイ状況を確認できます。
    - アイコンが緑色で表示された場合、正常にデプロイされた状態です。
    - アイコンが赤色で表示された場合、デプロイ中にタスクエラーが発生した状態です。デプロイに失敗した場合は、再デプロイを試みてください。失敗が続く場合は、カスタマーセンターへお問い合わせください。
...
### APIドキュメント
**ステージのデプロイ**を通じてデプロイされた構成を、APIドキュメントで確認できます。
詳細は[APIドキュメント](./console-guide/#api_2)をご参照ください。

## ステージプラグイン

### バックエンドエンドポイントURLの再定義

API Gatewayに受信したリクエストをバックエンドエンドポイントへ転送する際、デフォルトではステージに定義されたバックエンドエンドポイントURLへリクエストを転送します。
特定のパスまたはメソッドに対してバックエンドエンドポイントURLを再定義するには、バックエンドエンドポイントURLの再定義を設定します。

1. **ステージ**タブでステージを選択します。
2. **設定**タブを選択します。
3. ステージツリー画面で、バックエンドエンドポイントURLを再定義したいパスまたはメソッドを選択します。
4. バックエンドエンドポイントURLの再定義を有効化(on)します。
    - API Gatewayに受信したリクエストを転送する、バックエンドエンドポイントURLを作成します。
    - 下位パスを含めて作成できます。
        - 例：https://api.nhn.com , https://api.nhn.com/apis
    - URLにポートを直接指定する場合、80, 443, 10000～12000ポートのみ使用可能です。
...
5. Log & Crash Searchサービスを利用していない場合は、**サービス有効化**をクリックしてサービスを有効にします。 
6. **修正**ボタンをクリックして設定を保存します。
7. 再度**ステージ配布**を行った後からアクセスログがLog & Crash Searchサービスに保管されます。 
...
API GatewayへのAPIリクエスト時に、指定されたAPI Keyのみがリクエストできるよう制限します。

- 有効なAPI Keyの値であるか検証します。
- 使用量計画のステージに接続されたAPI Keyのみが、ステージのAPIにリクエストできます。詳細は、[使用量計画 > 使用量計画にステージを接続](./console-guide/#_48)をご参照ください。
- API Keyが接続された使用量計画のリクエスト上限を検証します。使用量計画のリクエスト上限を設定する方法の詳細は、[使用量計画 > 使用量計画の作成](./console-guide/#_45)をご参照ください。

> **[参考] API Keyの失敗レスポンス** <br>
> API Keyの値がリクエストヘッダに含まれていない、無効である、または使用量上限を超過した場合、APIリクエストは拒否されます。
> 詳細は、[Gatewayエラーコード](./error-code/)のドキュメントをご参照ください。
1. **ステージ**タブでステージを選択します。
2. **設定**タブを選択します。
3. ステージツリー画面で、API Keyを有効化したいパスまたはメソッドを選択します。
    - パスで設定したAPI Keyは、その下に定義された全てのパス及びメソッド呼び出しに対して適用されます。
4. API Keyを有効化(on)します。
5. ステージをデプロイします。
6. APIリクエスト時、`x-nhn-apikey`ヘッダにAPI Keyの値を追加してリクエストします。

| ヘッダ名 | ヘッダ値 |
| --- | --- |
| x-nhn-apikey | <primary api key または secondary api key\> |

### リクエスト制限ポリシー 

登録済みのリクエスト制限ポリシーを、ステージのリソースパスまたはメソッドに適用します。
詳細は、[リクエスト制限ポリシー](./console-guide/#_34)をご参照ください。

1. **ステージ**タブでステージを選択します。
2. **設定**タブを選択します。
3. ステージツリー画面で、リクエスト制限ポリシーを適用したいパスまたはメソッドを選択します。
4. リクエスト制限ポリシーを**有効化(On)**します。
5. 適用するリクエスト制限ポリシーを選択し、**修正**ボタンをクリックして保存します。
6. **ステージのデプロイ**をクリックします。ステージのデプロイが完了すると、設定されたリクエスト制限ポリシーが動作します。
...
1. リクエスト制限リストで、修正したいリクエスト制限ポリシーの**管理**列にある鉛筆アイコンをクリックします。
2. リクエスト制限ポリシーの情報を変更した後、**修正**ボタンをクリックします。


### リクエスト制限ポリシーの削除
1. リクエスト制限リストで、削除したいリクエスト制限ポリシーの**管理**列にあるごみ箱アイコンをクリックします。
2. **削除**ボタンをクリックします。
...
2.下部のタブで**リクエスト制限ポリシーキー値の作成**ボタンをクリックします。
...
2. **ゲートウェイレスポンス**で、設定したいゲートウェイレスポンスタイプの**管理**タブにある鉛筆アイコンをクリックします。
3. レスポンス情報を入力します。 
...
> **[参考]ゲートウェイレスポンスの適用** <br>
> 変更されたゲートウェイレスポンスは、ステージ配布後に配布時点の形状で適用されます。
### ゲートウェイレスポンス初期化
1. ゲートウェイレスポンスリストで、初期化したいエラータイプの**レスポンス**列にある**初期化**ボタンをクリックします。
2. 確認ウィンドウで**確認**ボタンをクリックします。初期化されたデータは復元できません。
...
## APIドキュメント
APIドキュメントを利用すると、API Gatewayに登録されたAPIの仕様をWebページのドキュメントとして管理できます。

### APIドキュメントの公開
APIドキュメントを公開する手順をご案内します。

1. リソース設定ページの**モデル**タブをクリックしてリクエスト、本文のモデルを登録します。
2. リソース設定ページでリソースメソッドを選択した後、リクエストパラメータとレスポンスを入力します。
3. **ステージ適用**ボタンをクリックし、ステージに変更されたリソース内容を適用します。
4. ステージ設定ページの**デプロイ**ボタンをクリックします。
5. ステージ設定ページの**APIドキュメントタブ**をクリックします。
6. APIドキュメントの公開タイプを選択します。
    - **公開タイプ**: APIドキュメントのアクセス制限を設定できます。
        - **公開**: アクセス制限なしでAPIドキュメントにアクセスできます。
        - **非公開**: コンソールにアクセス可能なユーザーのみがAPIドキュメントにアクセスできます。
7. **公開リンク**に表示されるURLで、APIドキュメントを確認できます。

> **[参考] ステージのデプロイとAPIドキュメントの公開** <br>
> - APIドキュメントは、初回のステージデプロイ後に公開され、最初の公開タイプは非公開に設定されます。
> - APIドキュメントは、ステージのデプロイを実行する際に、デプロイされる構成に更新されます。
>
> **[注意] 公開されたAPIドキュメントでのAPI呼び出しテスト時のCORS設定** <br>
> - 公開されたAPIドキュメントのドメインアドレスと、呼び出すAPIのドメインアドレスが異なるため、APIドキュメント内で呼び出しをテストしたい場合は、CORSの設定が必要になることがあります。
> - 例：
...
5. 右側の**ステージ統計**タブでは、ステージの統計指標を確認できます。
6. 右側の**リソース統計**タブでは、HTTPメソッドとパスごとに統計指標を確認できます。
...
- **API呼び出し**
    - **成功**: レスポンスされたHTTPステータスコードが2XX、3XXであるAPIの呼び出し数
    - **失敗**: レスポンスされたHTTPステータスコードが4XX、5XXであるAPIの呼び出し数
- **HTTP 2XX～5XXコード**：ステータスコードグループ別API呼び出し数
- **API Gatewayからすぐに返された数**：バックエンドエンドポイントサービスにリクエストを伝達せず、API Gatewayから返されたAPI呼び出し数
- **平均レスポンス時間**：API Gatewayにリクエストが入った後、APIリクエストクライアントに返すまでの平均時間(ms)
- **ネットワークアウトバウンドトラフィック**：API GatewayからAPIリクエストクライアントへ返されたデータのバイトサイズ
...
### 使用量計画の修正
1. 使用量計画リストで、修正したい使用量計画の管理列にある鉛筆アイコンをクリックします。
2. 使用量計画の情報を修正します。
3. 変更内容を保存するには、**修正**ボタンをクリックします。
...
### 使用量計画の削除
1. 使用量計画リストで、削除したい使用量計画の管理列にあるごみ箱アイコンをクリックします。
2. 削除確認ウィンドウで**確認**ボタンをクリックします。削除されたデータは復元できません。
...
### 使用量計画に接続されたステージの解除
1. 使用量計画リストで、接続を解除したい使用量計画の名前列にある名前のリンクをクリックします。
2. 接続済みステージリストで、解除したいステージの**接続解除**ボタンをクリックします。
3. 確認ウィンドウで**確認**ボタンをクリックします。
...
3. 下部のリストから接続解除するAPI Keyを選択した後、**接続解除**ボタンをクリックします。
4. 確認ウィンドウで**解除**ボタンをクリックします。

### API Keyの使用量計画の変更
1. 使用量計画リストから使用量計画名列の**名前**のリンクをクリックします。
2. 接続されたステージリストから使用量計画を変更するAPI Keyが存在するステージを選択します。
3. 下部のリストから使用量計画を変更するAPI Keyの**使用量計画変更**ボタンをクリックします。
...
1. API Key一覧ページで**インポート**ボタンをクリックした後、**CSVファイル選択**ボタンをクリックしてインポートするCSVファイルを選択します。
...
### API Keyの修正
1. API Keyリストで、修正したいAPI Keyの管理列にある鉛筆アイコンをクリックします。
2. API Keyの情報を修正します。修正可能な項目は、API Keyの名前、API Keyの説明、API Keyのステータスです。
3. 設定を変更した後、**修正**ボタンをクリックします。

> **[注意] API Keyの状態変更** <br>
> API Keyの状態をINACTIVEに変更すると、該当API Keyを使用できません。

### API Keyの削除
1. API Keyリストで、削除したいAPI Keyの管理列にあるごみ箱アイコンをクリックします。
2. 確認ウィンドウで**確認**ボタンをクリックします。削除されたデータは復旧できません。
...
1. **ユーザー指定ドメイン**メニューに移動します。
2. 削除したいカスタムドメインの管理列にあるごみ箱アイコンをクリックします。
3. 確認ウィンドウで**確認**ボタンをクリックします。削除されたデータは復元できません。
...
3. 右側の未接続ドメインリストから、接続したいカスタムドメインを選択します。
4. 確認ウィンドウで**確認**ボタンをクリックします。接続されたドメインは、接続済みドメインに移動します。

### ユーザー指定ドメインのステージ接続解除

1. ユーザー指定ドメインを接続解除するステージに移動します。
2. **ステージ**タブ > **ユーザー指定ドメイン**タブに移動します。 
3. 左側の接続済みドメインリストから、接続を解除したいカスタムドメインを選択します。
4. 確認ウィンドウで**確認**ボタンをクリックします。接続が解除されたドメインは、未接続ドメインに移動します。
...
2. 韓国((ピョンチョン)リージョンのAPI Gatewayサービスに運営中の韓国((パンギョ)リージョンステージと同じリソースを登録します。  
簡単にリソースを以前するには[ステージ > リソースのインポート](./console-guide/#_20)を通じてステージに登録されたリソースをSwaggerファイルでダウンロードした後、 [リソース > リソースのインポート](./console-guide/#_170)を通じてダウンロードしたSwaggerファイルでリソースを登録できます。 
3. 韓国((ピョンチョン)リージョンのステージを作成し、必要なステージ設定があれば修正後、ステージを配布します。
...
GSLB設定の詳細は、[DNS Plusコンソール使用ガイド](/Network/DNS%20Plus/ja/console-guide/)を参照してください。

1. DNS Plusサービスに移動します。
2. GSLBを作成します。 
3. ヘルスチェックを作成します。 
    * ヘルスチェックパスは韓国(パンギョ)、韓国(ピョンチョン)リージョンのAPI Gatewayにヘルスチェックに利用するリソースメソッドを構成してヘルスチェックリソースパスに登録します。
    * ヘルスチェックに失敗したPoolはサービスから除外されます。
4. Poolを作成します。 
    * Pool作成のヘルスチェックは、上記で作成したヘルスチェックを選択します。 
    * エンドポイントアドレスは韓国((パンギョ)リージョン、韓国(ピョンチョン)リージョンのAPI Gatewayステージドメインを入力します。 
    * 重みと状態は構成する冗長化構造(Active-Active、Active-StandBy)に応じて設定します。
5. GSLBと連結されたPoolがACTIVE状態であることを確認します。

### 3. ユーザー指定ドメインの作成 
1. 韓国(パンギョ)と韓国(ピョンチョン)リージョンでステージAPI呼び出し時に共通で利用するユーザー指定ドメインを作成します。リージョンごとにユーザー指定ドメインを作成する必要があり同じユーザー指定ドメインで作成する必要があります。 
2. ユーザー指定ドメインを作成する際、GSLBドメインに作成したGSLBドメインを入力します。 

### 4. ユーザー指定ドメインを各リージョンのステージに接続
1. 韓国(パンギョ)と韓国(ピョンチョン)リージョンの各API Gatewayステージに作成したユーザー指定ドメインを接続します。詳細な方法は[ユーザー指定ドメインをステージに接続ガイド](./console-guide/#_53)を参照してください。
2. ユーザー指定ドメインでAPIが正常に呼び出されるか、GSLB構成によってトラフィックが分散されるかを確認します。 


## API Gatewayサービスゲートウェイ連動 
サービスゲートウェイを利用すると、NHN Cloud内部でクライアントとAPI Gatewayが通信する際、外部インターネットを経由せず、内部ネットワークで通信できます。
API Gatewayサービスゲートウェイを連動する方法は、次のガイドを参照してください。


1. API Gatewayサービス用のサービスゲートウェイを作成します。
    * サービスゲートウェイを作成する方法はService Gateway > [コンソール使用ガイド](/Network/Service%20Gateway/ja/console-guide/)を参照してください。
2. /etc/hostsファイルにサービスゲートウェイIPとアクセスしたいステージのドメインアドレスを追加します。 
    * 例: 192.168.1.42 kr1-gateway-example.api.nhncloudservice.com 

> **[参考]/etc/hostsファイル修正** <br>
> /etc/hostsファイルを修正しないと、ステージURLでAPI呼び出しができません。
> 1つのサービスゲートウェイIPで複数のAPI Gatewayステージドメインを登録し、使用できます。
